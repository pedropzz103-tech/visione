...
function catalogSection({ id, eyebrow, title, cards, note = "", locale = "pt", className = "" }) {
  ...
  return `<section id="${id}" class="content-section catalog-section ${className}" data-catalog-section><div class="section-heading"><div><p class="eyebrow">${escapeHtml(eyebrow)}</p><h2>${escapeHtml(title)}</h2>${note ? `<p class="collection-note">${escapeHtml(note)}</p>` : ""}</div><div class="rail-controls"><button type="button" class="rail-view-button" data-rail-view data-open-label="${labels.open}" data-close-label="${labels.close}" aria-expanded="false">${labels.open}</button><div class="rail-arrows"><button type="button" data-rail-prev aria-label="${labels.previous}">←</button><button type="button" data-rail-next aria-label="${labels.next}">→</button></div></div></div><div class="title-rail cinematic-title-rail" data-catalog-rail tabindex="0">${cards}</div></section>`;
}
