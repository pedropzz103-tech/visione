...
function search(locale = "es", compact = false) {
  ...
  return `<div class="search-module${compact ? " search-compact" : ""}" data-visione-search data-locale="${locale}"><form role="search" action="${localePath(locale)}" class="search-form"><label class="sr-only" for="search-${locale}">${escapeHtml(config.searchPlaceholder)}</label><span class="search-icon" aria-hidden="true">⌕</span><input id="search-${locale}" data-search-input type="search" autocomplete="off" placeholder="${escapeHtml(config.searchPlaceholder)}"><button type="submit">Buscar</button></form><div class="search-results" data-search-results hidden></div></div>`;
}

function editorialCover(title, locale) {
  const localized = title.titles?.[locale] || title.original_title || title.slug;
  const mediaLabel = title.type === "series" ? (locale === "es" ? "Serie" : "Série") : (locale === "es" ? "Película" : "Filme");
  const genre = title.genres?.[0] || mediaLabel;
  return `<div class="editorial-cover" role="img" aria-label="Capa editorial VISIONE de ${escapeHtml(localized)}"><span class="editorial-mark">VISIONE EDITORIAL</span><strong>${escapeHtml(localized)}</strong><span class="editorial-meta"><span>${escapeHtml(String(title.year ?? ""))}</span><span>${escapeHtml(mediaLabel)}</span><span>${escapeHtml(genre)}</span></span></div>`;
}
...
function poster(title, locale) {
  ...
  return editorialCover(title, locale);
}
...
function offerGroup(title, locale, providers, monetization, heading) {
  ...
  return `<section class="offer-group"><h3>${heading}</h3><div class="offer-list">${offers.map((offer) => {
    ...
    const rel = offer.sponsored || offer.is_affiliate ? "nofollow sponsored" : "nofollow";
    return `<div class="offer-row"><div><span class="provider-badge">${escapeHtml(label.slice(0, 2).toUpperCase())}</span><strong>${escapeHtml(label)}</strong>${offer.sponsored ? '<small class="sponsored">Patrocinado</small>' : ""}</div><div>${price ? `<span class="offer-price">${escapeHtml(price)}</span>` : `<span class="offer-type">${monetization === "subscription" ? "Incluído na assinatura" : monetization === "free" ? "Grátis / com anúncios" : heading}</span>`}${href ? `<a class="offer-link" href="${escapeHtml(href)}" rel="${rel}">Ver oferta</a>` : ""}</div></div>`;
  }).join("")}</div></section>`;
}
...
export function renderTitlePage(title, locale, providers) {
  ...
  const freshness = title.availability_updated_at ? new Intl.DateTimeFormat(config.lang, { dateStyle: "long", timeZone: "UTC" }).format(new Date(title.availability_updated_at)) : null;
  ...
  const faq = state === "unknown" || state === "error" ? [
    { q: locale === "es" ? `¿Dónde ver ${name} en ${config.label}?` : `Onde ver ${name} em ${config.label}?`, a: state === "error" ? (locale === "es" ? "Los datos de disponibilidad están temporalmente no disponibles." : "Os dados de disponibilidade estão temporariamente indisponíveis.") : config.dataPendingLabel },
  ...
  const groups = [
    ...
    offerGroup(title, locale, providersMap, "buy", "Compra")
  ].filter(Boolean).join("");

  let availabilityBlock;
  if (countryOffers.length) {
    availabilityBlock = `${bestOffer(title, locale, providersMap)}${groups}`;
  } else if (state === "error") {
    availabilityBlock = `<div class="availability-empty pending"><span aria-hidden="true">◌</span><div><strong>${locale === "es" ? "Datos temporalmente no disponibles" : "Dados temporariamente indisponíveis"}</strong><p>${locale === "es" ? "No podemos confirmar las plataformas ahora mismo. No interpretamos este fallo como ausencia de ofertas." : "Não conseguimos confirmar as plataformas neste momento. Esta falha não é tratada como ausência de ofertas."}</p></div></div>`;
  } else {
    availabilityBlock = `<div class="availability-empty ${state === "unknown" ? "pending" : ""}"><span aria-hidden="true">${state === "unknown" ? "◌" : "✓"}</span><div><strong>${state === "unknown" ? (locale === "es" ? "Datos pendientes de fuente comercial" : "Dados aguardando fonte comercial") : (locale === "es" ? "Sin oferta verificada" : "Sem oferta verificada")}</strong><p>${escapeHtml(state === "unknown" ? config.dataPendingLabel : config.unavailableLabel)}</p></div></div>`;
  }

  const attributions = [...new Set(countryOffers.flatMap((offer) => Array.isArray(offer.attribution) ? offer.attribution : []).map(String).map((value) => value.trim()).filter(Boolean))];
  const attributionBlock = attributions.length ? `<p class="source-attribution"><strong>${locale === "es" ? "Fuente de disponibilidad:" : "Fonte de disponibilidade:"}</strong> ${attributions.map(escapeHtml).join(" · ")}</p>` : "";
