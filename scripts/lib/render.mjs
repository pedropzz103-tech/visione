...
export function renderLocalePage({ locale, titles, providers }) {
  ...
  return shell({ locale, path: localePath(locale), title: `VISIONE ${LOCALES[locale].country} | ${copy.promise}`, description: `${copy.promise} Pesquisa rápida com disponibilidade legal e fontes visíveis.`, body, alternates: hreflangForLocales(), pageClass: "discovery-page" });
}
...
export function renderTitlePage({ locale, title, providers }) {
  ...
  const faqItems = [
    { question: `${copy.where} ${name} em ${country}?`, answer: providerNames.length ? `A fonte confirma ${providerNames.join(", ")} no snapshot atualizado em ${updated}.` : "A VISIONE ainda não possui confirmação atual suficiente para este mercado." },
    { question: `A VISIONE transmite ${name}?`, answer: "Não. A VISIONE organiza informação e encaminha para opções legais confirmadas; não hospeda nem reproduz o título." },
  ];
  const schemas = [
  ...
  const faq = `<section class="detail-section faq"><p class="eyebrow">PERGUNTAS FREQUENTES</p><h2>Respostas diretas.</h2>${faqItems.map((item) => `<details><summary>${esc(item.question)}</summary><p>${esc(item.answer)}</p></details>`).join("")}</section>`;
