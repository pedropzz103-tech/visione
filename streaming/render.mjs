...
function localeSwitcher(active = null) {
  return `<div class="market-switcher" aria-label="Mercados">${SUPPORTED_LOCALES.map((locale) => {
    const config = getLocale(locale);
    return `<a href="${localePath(locale)}" data-market="${locale}"${active === locale ? ' aria-current="page"' : ""}><span aria-hidden="true">${config.flag}</span>${config.label}</a>`;
  }).join("")}</div>`;
}
...
function header(locale = null) {
  ...
  return `<header class="stream-header"><div class="stream-header-inner">${brand(locale)}<nav aria-label="Principal"><a href="${movies}">${locale === "es" ? "Películas" : "Filmes"}</a><a href="${series}">${locale === "es" ? "Series" : "Séries"}</a><a href="/news/">Wire</a></nav><div class="desktop-markets">${localeSwitcher(locale)}</div></div></header>`;
}
...
export function renderLocaleHome(locale, titles, providers) {
  ...
  return `<!doctype html><html lang="${config.lang}"><head>${pageHead({ title, description: `${config.promise} Compara streaming, aluguer/alquiler e compra quando os dados verificados estiverem disponíveis.`, canonical: absoluteUrl(localePath(locale)), hreflang: SUPPORTED_LOCALES.map((code) => ({ lang: getLocale(code).lang, href: absoluteUrl(localePath(code)) })) })}</head><body class="stream-body">${header(locale)}<main><section class="locale-hero"><div><p class="eyebrow">VISIONE · ${config.label.toUpperCase()}</p><h1>${escapeHtml(config.promise)}</h1><p>${locale === "es" ? "Busca un título y ve directamente a su ficha local." : "Procura um título e abre a ficha do teu mercado."}</p></div>${search(locale)}<div class="mobile-markets">${localeSwitcher(locale)}</div></section><section id="filmes" class="content-section"><div class="section-heading"><div><p class="eyebrow">${locale === "es" ? "PELÍCULAS" : "FILMES"}</p><h2>${locale === "es" ? "Para explorar ahora." : "Para explorar agora."}</h2></div></div><div class="title-rail">${movieCards}</div></section><section id="series" class="content-section"><div class="section-heading"><div><p class="eyebrow">${locale === "es" ? "SERIES" : "SÉRIES"}</p><h2>${locale === "es" ? "Historias para seguir." : "Histórias para acompanhar."}</h2></div></div><div class="title-rail">${seriesCards || `<p class="empty-state">${locale === "es" ? "Más series llegarán con la primera ingestión de datos." : "Mais séries entram com a primeira ingestão de dados."}</p>`}</div></section><section class="content-section"><div class="section-heading"><div><p class="eyebrow">STREAMING</p><h2>${locale === "es" ? "Servicios compatibles." : "Serviços compatíveis."}</h2></div></div>${providerChips(localProviders, locale)}</section></main>${footer(locale)}</body></html>`;
}
