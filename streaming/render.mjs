...
export function renderLocaleHome(locale, titles, providers) {
  ...
  return `<!doctype html><html lang="${config.lang}"><head>${pageHead({ title, description: `${config.promise} Compara streaming, aluguer/alquiler e compra quando os dados verificados estiverem disponíveis.`, canonical: absoluteUrl(localePath(locale)), hreflang: SUPPORTED_LOCALES.map((code) => ({ lang: getLocale(code).lang, href: absoluteUrl(localePath(code)) })) })}</head><body class="stream-body discovery-home">${header(locale)}<main><section class="locale-hero cinematic-hero"><div class="cinematic-backdrop" aria-hidden="true"><img src="/assets/visione-cinematic-hero-v2.png" alt="" fetchpriority="high"></div><div class="cinematic-overlay" aria-hidden="true"></div><div class="hero-content"><p class="eyebrow">${locale === "es" ? "PELÍCULAS · SERIES · DÓNDE VER" : locale === "br" ? "FILMES · SÉRIES · ONDE ASSISTIR" : "FILMES · SÉRIES · ONDE VER"}</p><h1>${escapeHtml(heroTitle)}</h1><p class="hero-lead">${escapeHtml(heroLead)}</p>${search(locale)}<p class="hero-note">${locale === "es" ? "Búsqueda simple. Opciones legales. Fuente identificada." : "Busca simples. Opções legais. Fonte identificada."}</p></div></section><section id="filmes" class="content-section discovery-section"><div class="section-heading"><div><p class="eyebrow">${locale === "es" ? "PELÍCULAS" : "FILMES"}</p><h2>${locale === "es" ? "Historias para tu lista." : "Histórias para a tua lista."}</h2></div></div><div class="title-rail cinematic-title-rail">${movieCards}</div></section><section id="series" class="content-section"><div class="section-heading"><div><p class="eyebrow">${locale === "es" ? "SERIES" : "SÉRIES"}</p><h2>${locale === "es" ? "Tu próxima obsesión." : "A tua próxima obsessão."}</h2></div></div><div class="title-rail cinematic-title-rail">${seriesCards || `<p class="empty-state">${locale === "es" ? "Más series llegarán con la primera ingestión de datos." : "Mais séries entram com a primeira ingestão de dados."}</p>`}</div></section><section class="content-section provider-section"><div class="section-heading"><div><p class="eyebrow">STREAMING</p><h2>${locale === "es" ? "Una búsqueda. Varias plataformas." : "Uma busca. Várias plataformas."}</h2></div></div>${providerChips(localProviders, locale)}</section></main>${footer(locale)}</body></html>`;
}
